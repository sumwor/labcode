...
# 2/23/2017 correc succeed probability of geometric distribution, previously for human is 1, now is 1/11~0.0909
#2/24/2017 
#-------HEADER PARAMETERS-------#
...
int block_Index=parameter_window.add_parameter("block");
...
string side;
string state;
double shift_threshold = 0.3; # sucess probability = 1/mean
double m=random();
##...to be continued
...
parameter_window.set_parameter(block_Index,string(block));
...
parameter_window.set_parameter(trialnumIndex,string(n_trial)+"m="+string(double(100)*m));	

parameter_window.set_parameter(hiP_rateIndex,string(nTrials_hiP_total*100/n_trial)+"%"+string(nTrials_hiP)+"/per b");
...
string asd = date_time("yymmddhhmm");
	ofile.open("C:\\Users\\KWANLAB\\Desktop\\Presentation\\logfiles\\logstats_wanyu_accuracy_2_21_no_lick.txt");
	#ofile.open_append("logstats_ITI.txt"); 
	ofile.print("\nStarting time:");
	ofile.print(date_time());
	ofile.print("\n\t Rig 2");
	ofile.print(string(shift_threshold));
